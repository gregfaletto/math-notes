...
These notes are based on my notes from chapter 8 of \textit{Time Series and Panel Data Econometrics} (1st edition) by M. Hashem Pesaran and coursework for Economics 613: Economic and Financial Time Series I at USC, as well as Math 505A and Math 541A at USC and chapter 7 from \textit{Probability and Random Processes} (Grimmett and Stirzaker) 3rd edition.
...
\]
\end{lemma}
\begin{proof} Note \( a \cdot \boldsymbol{1}_{\{|X| \geq a\}} \leq |X|\), where \(\boldsymbol{1}\) is the indicator function. Dividing both sides by \(a\) and taking expectations yields the result. \end{proof}
...
\item \begin{theorem} \label{asym.cauchy.schwarz} \textbf{Cauchy-Schwarz} (and \textbf{Bunyakovsky).} If \(X\) and \(Y\) are random variables with finite variance then
...
\]
 ...
 \begin{theorem} \textbf{Jensen's Inequality} (Grimmett and Stirzaker p.181, 349) Let \(X: \Omega \to [-\infty, \infty]\) be a random variable. Let \(u: \mathbb{R} \to \mathbb{R}\) be convex. If \(\E |X| < \infty\) and \(\E|u(X)| < \infty\), then
...
\begin{proof} Hint: use Proposition \ref{asym.conv.prop} with \(y := \E(X)\).

%Since \(u\) is convex, we have \(u(tx + (1-t)y) \leq t u(x) + (1-t) u(y)\) for any \(x, y \in \mathbb{R}\) and for any \(t \in [0,1]\).

\end{proof}

\begin{corollary} \textbf{Triangle Inequality.} Let \(X: \Omega \to [-\infty, \infty]\) be a random variable. If \(\E |X| < \infty\), then \(|\E(X)| \leq \E(|X|)\).

\end{corollary}

\begin{proof} Follows immediately from Jensen's Inequality using \(u(X) = |X|\). \end{proof}

\begin{corollary}If \(\E(X^2) < \infty\), then \(\E(|X|) < \infty\), so \(\E(X) \in \mathbb{R}\). \end{corollary}

\begin{proof} Follows immediately from Jensen's Inequality using \(u(X) = X^2 \).\end{proof}

\item \begin{theorem}\label{asym.thm.holder.ineq} \textbf{Holder} (Grimmett and Stirzaker p. p. 143, 319) Generalization of Cauchy-Schwarz. For \(p, q > 1\) satisfying \(1/p + 1/q =1\) we have
...
\item \begin{theorem}\textbf{Triangle Inequality:} Let \(X, Y: \Omega \to \mathbb{R}\) be random variables. Let \(1 \leq p \leq \infty\). Then
...
\begin{theorem}\label{asym.wlln} \textbf{Weak Law of Large Numbers (Khinchine) (Pesaran 8.6 Theorem 10, Grimmett and Stirzaker Theorem 7.4.7).} Suppose that \(\{X_k\}\) is a sequence of (i) IID random variables with (ii) constant means, i.e., \(\E(X_k) = \mu < \infty\). Then
...
\begin{theorem}\label{asym.lln2} \textbf{Strong Law of Large Numbers (Grimmett and Stirzakker Theorem 7.5.1).} Let \(\{X_k\}\) be a sequence of (i) independent (ii) identically distributed random variables. Then if and only if (iii) \(\E|X_k|  < \infty \),
