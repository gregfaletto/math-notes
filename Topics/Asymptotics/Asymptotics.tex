...
\item \begin{theorem} \label{asym.cauchy.schwarz} \textbf{Cauchy-Schwarz.} (and \textbf{Bunyakovsky})
...
so we have \(\Pr(X = \E(X)) = 1\).
\end{proof}

We are now ready to prove the Cauchy-Schwarz Inequality.

\begin{proof}
 if \(\E(X^2) = 0\) or \(\E(Y^2)=0\), the Cauchy-Schwarz Inequality follows immediately, because that would imply that the right side is 0 and either \(X\) or \(Y\) is constant so the left side is zero too. In the case that \(\E(X^2) > 0\) and \(\E(Y^2) > 0\), for \(a, b \in \mathbb{R}\), let \(Z = aX - bY\). Then 
...
which is true if and only if \(\Pr(aX = bY) = 1\) by Lemma (\ref{asym.cauchy.lemma}).
\end{proof}
