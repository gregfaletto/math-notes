...
\item \begin{theorem}[\textbf{Chebyshev's Inequality} (probability p. 319)]\label{asym.cheby} Let \(X: \Omega \to [-\infty, \infty]\) be an (integrable) random variable with \(\E(X^2) < \infty\).Then for any real number \(k > 0\)
...
\begin{theorem}[\textbf{(Theorem 7.2.3, Grimmett and Stirzaker.)}]\label{asym.convg.implications} The following implications hold:
...
% \(\E(|Y_n - Y|^2) \leq \lVert Y_n - Y \rVert_\infty\).
...
\textbf{Other theorems to include:} Kolmogorov's Maximal Inequality, Kolmogorov Three-Series Test, Lindeberg Feller Central Limit Theorem, \textbf{this and more at beginning of Mike's 505A qual solutions.}
...
%\textbf{Definitely need to study this one more closely.}
...
\section{Worked Examples from Math 505A Midterm 2}
