...
\subsection{High dimensional correlations}
...
%\begin{align*}
%1 & = \E \left[ \sum_{i=1}^n y_i^2 \right] =    \sum_{i=1}^n \E \left[ y_i^2 \right]  = n \E \left[ y_i^2 \right]
%\end{align*}
%(where we used exchangeability of all \(y_i\) in the last step). So
%\[
%\Var( x^\top y) =  \Var\left( y_1 \right) = \E[y_1^2] - \E[y_1]^2 = 1/n.
%\]
%Notice that since \((x^\top y)^2 = y_1^2 \leq 1\) with probability 1, (using the \(\psi_2\) norm from Definition 2.5.6 of \citealt{vershynin2018high})
%\begin{align*}
%\lVert x^\top y \rVert_{\psi_2} & = \inf \left\{ t > 0: \E \exp \left\{ \frac{( x^\top y)^2}{t^2} \right\} \leq 2 \right\}
%\\ & = \inf \left\{ t > 0: \E \exp \left\{ \frac{y_1^2}{t^2} \right\} \leq 2 \right\}
%\\ & \leq \inf \left\{ t > 0: \exp \left\{ \frac{1}{t^2} \right\} \leq 2 \right\}
%\\ & = \frac{1}{\sqrt{ \log 2}}.
%%\\ & =inf \left\{ t > 0: t^2  \leq  \log 2 \right\}
%\end{align*}
%%which is given by \(t = 1/\sqrt{\log 2}\).
%Then by the General Hoeffding's Inequality (Theorem 2.6.2 in \citealt{vershynin2018high}), there exists a finite \(c > 0\) such that
%\[
%\mathbb{P} \left(  \left| x^\top y \right| \geq \epsilon \right)  \leq 2 \exp \left\{ - \frac{c \epsilon^2}{\lVert x^\top y \rVert_{\psi_2}^2} \right\} = 2 \exp \left\{ - c  \sqrt{\log 2} \epsilon^2  \right\}
%\]
%\end{proof}
\begin{proposition}
Suppose \(x\) and \(y\) are vectors drawn from the uniform distribution on \(\mathcal{S}^{n-1}\), the unit sphere in \(\mathbb{R}^n\). Then for any \(\epsilon > 0\),
...
%\begin{proof}
%\[
%\E \left[  \left| x^\top y \right| \right] = \sum_{i=1}^n \E \left[ x_i y_i \right] \leq   \sum_{i=1}^n  \sqrt{\E \left[ x_i^2 \right] \E \left[ y_i^2 \right] } = n \sqrt{\E \left[ x_1^2 \right] \E \left[ y_1^2 \right] } = n \cdot \sqrt{ \frac{1}{n} \cdot \frac{1}{n} }= 1.
%\]
%\end{proof}
...
%  \(\boldsymbol{X}^T\boldsymbol{X}\) must be positive semidefinite. In fact, it is positive definite because \( \boldsymbol{X} \in \mathbb{R}^{n \times p}\) has full rank; that is, \(\operatorname{rank}(\boldsymbol{X}) = p\), so \(\boldsymbol{X}^T\boldsymbol{X} \in \mathbb{R}^{p \times p}\) has rank \(p\) (full rank) and is invertible. So \(\boldsymbol{X}^T\boldsymbol{X}\) is positive definite (all positive eigenvalues). Then since \(\Tr(\boldsymbol{X}^T\boldsymbol{X} + \lambda \boldsymbol{I}_p) > \Tr(\boldsymbol{X}^T\boldsymbol{X})\), the eigenvalues of \(\boldsymbol{X}^T\boldsymbol{X} + \lambda \boldsymbol{I}_p\) are also all positive, which means the determinant of \(\boldsymbol{X}^T\boldsymbol{X} + \lambda \boldsymbol{I}_p\) is nonzero, which means it is invertible.
